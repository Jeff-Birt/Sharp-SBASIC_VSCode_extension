...
exports.token152 = token152;
const hdr158_1 = require("./hdr158");
...
var Mode;
(function (Mode) {
    Mode[Mode["LineNum"] = 0] = "LineNum";
    Mode[Mode["Symbol"] = 1] = "Symbol";
    Mode[Mode["KeyVar"] = 2] = "KeyVar";
    Mode[Mode["REMQte"] = 3] = "REMQte";
    Mode[Mode["Error"] = 4] = "Error"; // A parsing error occured
})(Mode || (Mode = {}));
...
let oaHead = 0; // start of new line in ouputArray
let oaIndex = 0; // index of next free byte in outputArray
let searchMode = Mode.LineNum; // start out searching for line #
let numFound = -1; // flag start of line # found
//token152(data);
// ----------------------------------------------------------------------------
// Tokenizes plain text file passed, calls hdr15 to add CE-158 header
function token152(fileName, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        let nxtChAsc = data.charCodeAt(i);
        ...
        if (searchMode == Mode.LineNum) {
            if (((nxtChAsc >= 0x30) && (nxtChAsc <= 0x39)) && (numFound == -1)) {
                ...
                if (numFound > -1) {
                    // have a valid line#
                    let returnedVal = Number(data.substring(stIndex, i));
            ...
            if (nxtChAsc == 0x22) {
                ...
                let match = data.substring(stIndex, i + 1);
            ...
            let match = data.substring(stIndex, i + 1);
            let tokIndex = keywordArray.indexOf(match); // Returns index of keyword or -1 if no match
            // Handle keyword match or miss
            if (tokIndex > -1) {
                // Add keyword to output, change mode
                stIndex = i + 1;
                let tok = tokenArray[tokIndex]; // token hex value
...
//# sourceMappingURL=token15.js.map