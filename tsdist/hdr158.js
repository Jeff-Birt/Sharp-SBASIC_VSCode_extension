...
exports.header158 = header158;
//import * as filStrm from 'fs';
let fName = ""; // file name
let type = ""; // file type bin, bas, rsv
let outName = ""; // output file name
let outExt = ""; // temp hold new file extension
let address = 0; // starting address
let marker = 32; // file type marker
//-----------------------------------------------------------------------------
//
function header158(fName, type, data, address) {
    ...
    if (fName.length > 13) {
        fName = fName.slice(0, 12);
    } // trim name if too long 12.3
    let xPos = fName.lastIndexOf("."); // will be start of file extension
    if (xPos == -1) {
        xPos = fName.length;
    } // if no '.'
    ...
    let fLen = data.length - 1; // file name length
...
//# sourceMappingURL=hdr158.js.map