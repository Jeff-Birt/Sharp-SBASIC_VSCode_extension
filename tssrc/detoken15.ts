...
class FileInfo
{
    ...
    public decode: boolean = false;
    public fileType: string = "";
    public progName: string = "";
    public stAddress: number = 0;
    public progLen: number = 0;

    public topROM: number = 0;
    public topBASIC: number = 0;
    public romSize: number = 0;
    public listOK: boolean = false;
   
    public saveInfo: boolean = false;   // -I save info file as fileName_Info.txt
    public detoke: boolean = false;     // -D Detokenize BASIC code save file as fileName.ASC
    public rDump: boolean = false;      // -RB Dump reserve file as binary file
    public rTxt: boolean = false;       // -RT Dump Reserve memory as decoded text

    public comHeader: string = "";      // holds decoded COM header
    public romHeader: string = "";      // holds decoded ROM header
    public resMem: string = "";         // holds Reserve Memory
    public basTxt: string = "";         // BASIC file detokenized to plain text

    constructor(fileName: string, sourceFile: Buffer)
    {
...
export function fileInfo15(file: string, option: string): string
{   
    let fInfo: FileInfo = new FileInfo(file, filStrm.readFileSync(file));
    let typeStr: string = "";
    let index: number = 0;
...
function deToken15(data: Buffer, index: number): string
{
    let eol: number = 0;        // data.indexOf(0x0D, index+3); // +3 to 
    let dPoint: number = index; // points to current locaito in data buffer
    let prgOut: string = "";    // program in ASCII
    ...
    while (dPoint < data.length)
    {
        ...
        let lineLen: number = data[dPoint+2]; //
        let lineLenOK: boolean = (eol - (dPoint + 2 + lineLen)) == 0; // check lineLen against eol
        ...
        if (lineNumValid(lineNum) == true && lineLenOK)
        {
            ...
            while (dPoint < eol)
            {
                ...
                let tok: number = data[dPoint]*256 + data[dPoint+1];
...
function decodeCOMHeader(index: number, fInfo: FileInfo): string
{
    fInfo.decode = false;
    ...
    else if (fInfo.fileType == "@COM")
    {
        ...
        index = 27; // look for ROM header
        let lineLen: number = fInfo.sourceFile[index+2]; // line length byte
        let lineLenOK: boolean = (lineLen - fInfo.sourceFile.indexOf(0x0D, index+3)) != 0;
...
function decodeROMHeader(index: number, fInfo: FileInfo, preStr: string): string
{
    let typeStr: string = "";
    ...
    if (fInfo.sourceFile[index] == 0x55) 
    {
        ...
        let lineLen: number = fInfo.sourceFile[fInfo.topBASIC+2];//
        let lineLenOK: boolean = (lineLen - fInfo.sourceFile.indexOf(0x0D, fInfo.topBASIC+2)) != 0;
...
function getLineNum(data: Uint8Array): number
{
    let val: number = 0;   // 0 indicates failed conv to #
...
function lineNumValid(lineNum: number): boolean
{
